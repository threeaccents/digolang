...
func New(l *lexer.Lexer) *Parser {
	...
	p.registerInfix(token.PERIOD, p.parseSelectorExpression)
...
func (p *Parser) parseLetStatement() *ast.LetStatement {
	...
	// if expectPeek is true it moves to the next token in the lexer.
	// after a let token we always expect the next token to be an `IDENTIFIER`
	if !p.expectPeek(token.IDENT) {
		return nil
	}

	// Here we actually set the identifier along with its token. Since above expectPeek moved to the next token
	// we went from the let token to now the identifier token.
